...
if __name__=="__main__":
    ...
    f_root = "D:/SKTAI/Kaggle Fairytale/"
    ...
    df = pd.read_csv(f_root+"merged_clean.tsv", sep='\t', encoding='utf-8')
    st_curr = -1
    for idx, n, prompt, _ in tqdm(df.itertuples(index=False, name=None)):
        ...
        with autocast(device):
            image = pipe2(prompt=add_postfix(prompt, -4), init_image=init_img, num_inference_steps=60, strength=0.9, guidance_scale=7.5, generator=generator).images[0]
        print(prompt)
